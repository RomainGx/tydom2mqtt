...
class TydomWebSocketClient():
    ...
    async def connect(self):
        ...
        # Get first handshake
        conn.request("GET", "/mediation/client?mac={}&appli=1".format(self.mac), None, http_headers)
...
############ Utils
    ...
    def build_digest_headers(self, nonce):
        ...
        return digest_auth.build_digest_header('GET', "https://{}:443/mediation/client?mac={}&appli=1".format(self.host, self.mac))
    ...
    async def send_message(self, method, msg):
        ...
        if not 'pwd' in msg:
            ...
            print('>>>>>>>>>> Sending to tydom client.....', method, 'secret msg')
